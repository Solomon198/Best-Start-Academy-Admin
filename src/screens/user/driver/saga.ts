...
const getDrivers = async () => {
  return axios.get(`${configuration.API_ENDPOINT}/admin/drivers?page=1`);
};
...
const createDriverAccount = async (payload) => {
  return axios.post(`${configuration.API_ENDPOINT}/admin/add-driver`, payload);
};
...
function* watchAddDriverAccount() {
  yield takeEvery(
    AddDriverAccount.ADD_DRIVER_ACCOUNT_MANAGER_CALLER,
    function* (action: any) {
      try {
        yield put({
          type: AddDriverAccount.ADD_DRIVER_ACCOUNT_MANAGER_STARTED,
        });
        ...
        yield put({
          type: AddDriverAccount.ADD_DRIVER_ACCOUNT_MANAGER_SUCCESS,
        ...
        yield put({
          type: AddDriverAccount.ADD_DRIVER_ACCOUNT_MANAGER_FAILED,
          paylaod: message,
        });
        yield put({
          type: SnackBarActions.CONFIGURE_SNACKBAR,
...
function* watchGetDrivers() {
  yield takeEvery(Driver.GET_DRIVER_CALLER, function* (action: any) {
    try {
      yield put({ type: Driver.GET_DRIVER_STARTER });
      const driverData = yield call(getDrivers.bind(null));
      yield put({
        type: Driver.GET_DRIVER_SUCCESS,
        payload: driverData.data.payload,
      });
      console.log(driverData.data);
    } catch (e) {
      yield put({ type: Driver.GET_DRIVER_FAILED, payload: e.message });
    }
  });
}
